...
typedef enum { INFILE, RANDOM, INCREASING, DECREASING,
    INSERTION, MERGE, QUICK1, HEAP, QUICK2, QUICK3, EXIT}
    MenuOptionTypes;
...
void QuickSort1(long *, int ,int);             /* Quick Sort(pivot as first element) */
...
void QuickSort2(long *L, int first, int last);  /* (pivot as random) */
void QuickSort3(long *L, int first, int last);  /* (pivot as median) */
        ...
        switch(choice)    /* Examine value of user's choice.           */
        {
            ...
            case QUICK2:  /* (9) QUICK SORT */
                ...
                if( List != NULL )
                {
                    ...
                    ShowResults("2Quick Sort (pivot 2)", start, end, num, filename);
                    Check(TestList,num);  /* Check for correct sorting */
                    free(TestList);
                } else {
                    printf ("\nThere is no input list. Specify one!.\n");
            ...
            case QUICK3:  /* (10) QUICK SORT */
                ...
                if( List != NULL )
                {
                    ...
                    ShowResults("3Quick Sort (pivot 3)", start, end, num, filename);
                    Check(TestList,num);  /* Check for correct sorting */
                    free(TestList);
                } else {
                    printf ("\nThere is no input list. Specify one!.\n");
...
/**************************************************************************
PROCEDURE: QuickSort2 (*L, first, last)
This procedure uses the quick-sort method to sort elements.
It uses the random element in the array as the pivot element.
INPUT: L	   Pointer to unsorted array of elements
first  first element of chunk to sort this time
last   last element of chunk to sort this time
OUTPUT: L      Pointer to sorted array of elements
...
void QuickSort2(long *L, int first, int last)
{
    ...
    if( first < last )
    {
        /* Use random element as pivot (for splitting) */
        int mod=last-first+1;
        int t=rand()%mod;
        t+=first;
        SplitPoint = t;                 /* assign SplitPoint to 1st index */
        Split(L, first, last, &SplitPoint); /* Split List around SplitPoint */
        QuickSort2(L, first, SplitPoint-1);  /* Sort 1st section of list */
        QuickSort2(L, SplitPoint+1, last);   /* Sort 2nd section of list */
    }
}


/**************************************************************************
PROCEDURE: QuickSort3 (*L, first, last)
This procedure uses the quick-sort method to sort elements.
It uses the median element in the array as the pivot element.
INPUT: L	   Pointer to unsorted array of elements
first  first element of chunk to sort this time
last   last element of chunk to sort this time
OUTPUT: L      Pointer to sorted array of elements
...
void QuickSort3(long *L, int first, int last)
{
    ...
    if( first < last )
    {
        /* Use median element as pivot (for splitting) */
        //int t=FindMedian(L,first,(last-first+1)/2,last);
        //int t=FindMedian(L,first,(last+first)/2,last);
        int t=FindMedian(L,first,first+(last-first+1)/2,last);
        SplitPoint = t;                 /* assign SplitPoint to 1st index */
        Split(L, first, last, &SplitPoint); /* Split List around SplitPoint */
        QuickSort3(L, first, SplitPoint-1);  /* Sort 1st section of list */
        QuickSort3(L, SplitPoint+1, last);   /* Sort 2nd section of list */
    }
}
...
{
    long output[10000]; // output array
    int i, count[10] = {0};
    ...
    /* Displays user menu choices and returns value of user's choice */
{
    ...
    printf("(7) Heap Sort               \n");
    printf("(8) Quick Sort (pivot as random)  \n");
    printf("(9) Quick Sort (pivot as median)  \n");
    printf("(10) Exit                   \n");
